...
- The repo now also carries install artifacts under `GlassToKey.Linux/packaging/`: a checked-in `udev` rule template and an install script for published Linux builds with wrapper-vs-service install decisions.
- The repo now also carries a first Debian package skeleton under `GlassToKey.Linux/packaging/deb/`, and that path has been validated far enough to produce a `.deb` from the current publish outputs.
