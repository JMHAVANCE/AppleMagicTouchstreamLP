...
- Linux direct haptics pulse:
  - `dotnet run --project GlassToKey.Linux/GlassToKey.Linux.csproj -c Release -- pulse-haptics left 5`
  - `dotnet run --project GlassToKey.Linux/GlassToKey.Linux.csproj -c Release -- pulse-haptics right 5`
...
- Linux haptics has now been validated end-to-end on the current Ubuntu host for both tested USB trackpads:
...
- On this host, Linux actuator writes work with the raw 14-byte `0x53` report payload; do not assume the Windows 64-byte padded write shape is correct on Linux.
...
- The checked-in Linux packaging rules must grant the `glasstokey` group access to matched trackpad `event*` nodes, matched actuator `hidraw*` nodes, and `/dev/uinput`; haptics will not work in normal user mode if the actuator hidraw rule is missing.
