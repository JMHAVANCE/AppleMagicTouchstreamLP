...
- `GlassToKey.Linux.Gui/` now exists as an early Avalonia control shell on top of the same XDG-backed Linux host settings used by the CLI. It now supports device assignment, preset selection, keymap browse/clear, an in-app `doctor` report, runtime user-service start/stop/status control, and a first Ubuntu top-bar/tray shell through Avalonia `TrayIcon`. GUI publish now works both framework-dependent and self-contained.
